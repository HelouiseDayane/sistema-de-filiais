...
class EvolutionApiService
{
    ...
    public function connectionState(string $instanceName): array
    {
        try {
            ...
            if ($response->successful()) {
                $data = $response->json();
                // Evolution API retorna: {"instance":{"state":"open","instanceName":"..."}}
            ...
            // Se a instância não existe (404), retornar como fechada
            if ($response->status() === 404) {
                Log::warning('Instance not found in Evolution API', [
                    'instanceName' => $instanceName,
                ]);
                return [
                    'success' => true,

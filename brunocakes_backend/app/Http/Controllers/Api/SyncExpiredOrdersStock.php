...
class SyncExpiredOrdersStock extends Command
{
    ...
    public function handle()
    {
        ...
        // Limpa apenas reservas expiradas (TTL == -2) das reservas totais
        ...
        foreach ($allProductIds as $productId) {
            ...
            foreach ($keys as $key) {
                ...
                if ($ttl === -2) { // chave já expirou
                    // Recupera a quantidade que estava reservada (se possível)
                    $qty = Redis::get($key);
                    if ($qty > 0) {
                        $totalExpired += $qty;
                    }
            ...
            if ($totalExpired > 0) {
                Redis::decrby($reservedKey, $totalExpired);
                Redis::incrby("product_stock_{$productId}", $totalExpired);
                $this->info("Reserva expirada removida do produto #{$productId} (devolvido {$totalExpired} ao estoque)");
            }
        }
    }
}
